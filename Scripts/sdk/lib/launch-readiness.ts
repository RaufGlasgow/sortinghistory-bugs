...
export const TEST_BUG_REPORTS: TestBugReport[] = [
  {
 ...
 */
export async function submitVolumeTest(
  endpointUrl: string,
  reports: TestBugReport[],
 ...
 */
export function extractDigestActionUrls(html: string): DigestActionButton[] {
  const buttons: DigestActionButton[] = [];
 ...
 */
export function validateConcurrentStateFiles(
  stateDir: string,
  ...
  for (const expected of expectedWorkflows) {
    ...
    if (!fs.existsSync(filePath)) {
      contamination.push(
        `Missing state file: ${expected.workflow_id}.json`,
      );
      continue;
    }
    ...
    try {
      ...
      if (state.issue_number !== expected.issue_number) {
        contamination.push(
          `${expected.workflow_id}: issue_number is ${state.issue_number}, expected ${expected.issue_number}`,
        );
      }
      ...
      if (state.workflow_type !== expected.workflow_type) {
        contamination.push(
          `${expected.workflow_id}: workflow_type is ${state.workflow_type}, expected ${expected.workflow_type}`,
        );
      }
      ...
      const errMsg = err instanceof Error ? err.message : String(err);
      contamination.push(
        `${expected.workflow_id}: parse error: ${errMsg}`,
      );
    }
  }
 ...
 */
export function createTestStateFiles(
  stateDir: string,
  workflows: Array<{
    workflow_id: string;
    issue_number: number;
