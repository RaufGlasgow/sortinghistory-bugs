...
const BUILD_ARTIFACT_PATTERNS: readonly string[] = [
  "DerivedData/",
  ".build/",
  ".dSYM/",
];
...
const BUILD_ARTIFACT_EXTENSIONS: ReadonlySet<string> = new Set([
  ".o",
  ".dSYM",
  ".app",
]);
...
const TEMP_FILE_PATTERNS: readonly string[] = [
  "xcuserdata/",
];

/** File names that indicate temp files */
const TEMP_FILE_NAMES: ReadonlySet<string> = new Set([
  ".DS_Store",
]);

/** File extensions that indicate temp files */
const TEMP_FILE_EXTENSIONS: ReadonlySet<string> = new Set([
  ".tmp",
  ".swp",
]);

/** Path substrings that indicate automation files */
const AUTOMATION_PATH_PATTERNS: readonly string[] = [
  ".github/",
  "Scripts/",
  "node_modules/",
];
...
const AUTOMATION_EXTENSIONS: ReadonlySet<string> = new Set([
  ".yml",
  ".yaml",
  ".ts",
  ".js",
 ...
 */
function checkNoBuildArtifacts(changedFiles: string[]): QualityFailure | null {
 ...
 */
function checkNoTempFiles(changedFiles: string[]): QualityFailure | null {
  const offending: string[] = [];
 ...
 */
function checkNoAutomationFiles(changedFiles: string[]): QualityFailure | null {
  const offending: string[] = [];
 ...
 */
function checkDiffProportionality(
  diff: string,
  maxLines: number,
): QualityFailure | null {
 ...
 */
function checkNoBinaryFiles(diff: string): QualityFailure | null {
...
export function runQualityGate(
  ...
  const allowedExtensions = new Set(
    config?.allowedExtensions ?? DEFAULT_ALLOWED_EXTENSIONS,
  );
  ...
  if (buildArtifactResult) failures.push(buildArtifactResult);

  const tempFileResult = checkNoTempFiles(changedFiles);
  if (tempFileResult) failures.push(tempFileResult);

  const automationResult = checkNoAutomationFiles(changedFiles);
  if (automationResult) failures.push(automationResult);

  const proportionalityResult = checkDiffProportionality(diff, maxDiffLines);
  if (proportionalityResult) failures.push(proportionalityResult);

  const binaryResult = checkNoBinaryFiles(diff);
  if (binaryResult) failures.push(binaryResult);

  const extensionResult = checkAllowedExtensions(changedFiles, allowedExtensions);
  if (extensionResult) failures.push(extensionResult);
