 ...
 */
function resolveAndValidatePath(filePath: string, cwd: string): string {
...
async function runPreToolHooks(
  ...
  if (!preToolHooks || preToolHooks.length === 0) return null;
...
function executeRead(
  ...
  if (!filePath) return { success: false, output: "Missing required parameter: file_path" };
...
function executeEdit(
  ...
  if (!filePath) return { success: false, output: "Missing required parameter: file_path" };
  if (oldString === undefined) return { success: false, output: "Missing required parameter: old_string" };
  if (newString === undefined) return { success: false, output: "Missing required parameter: new_string" };
...
function executeGlob(
  ...
  if (!pattern) return { success: false, output: "Missing required parameter: pattern" };
...
function executeGrep(
  ...
  const searchPath = (args.path as string | undefined) ?? options.cwd;
...
function executeBash(
  ...
  if (!command) return Promise.resolve({ success: false, output: "Missing required parameter: command" });
  ...
  return new Promise((resolve) => {
    const child = exec(command, {
      ...
      if (error) {
        if (error.killed || (error as NodeJS.ErrnoException).code === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER") {
