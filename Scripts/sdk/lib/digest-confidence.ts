...
import type { RoutingDecisionLogEntry } from "./routing-log.js";
 ...
 */
export function readRoutingLogForDate(
  date?: string,
  repoRoot?: string,
  ...
  const byIssue = new Map<number, RoutingDecisionLogEntry>();
  for (const entry of entries) {
    byIssue.set(entry.issue, entry);
  }

  const needsAttention: IssueConfidenceData[] = [];
  const normal: IssueConfidenceData[] = [];
  ...
  for (const entry of byIssue.values()) {
    ...
    const data: IssueConfidenceData = {
      issue: entry.issue,
 ...
 */
export function buildLabelFallback(
  ...
  for (const issue of issues) {
    ...
    const classificationLabel = issue.labels.find(l =>
      l === ROUTING.LABEL_CONTENT_ERROR ||
      ...
      l === ROUTING.LABEL_NEEDS_HUMAN_REVIEW
    ) ?? "unknown";
 ...
 */
export function renderNeedsAttentionHtml(
  flaggedIssues: IssueConfidenceData[],
  issueUrlBase: string,
): string {
 ...
 */
export function formatClassificationDisplay(
  classification: string,
  confidence: number,
): string {
  if (confidence < 0) {
