...
import type { WorkflowState, WorkflowFinding } from "./state.js";
...
const GATE_SUGGESTIONS: Record<string, string> = {
  factual_accuracy:
    "Content verifier factual accuracy gate failing repeatedly -- review Scripts/sdk/prompts/content-verifier.md for calibration",
  proof_check:
    "Proof check gate failing repeatedly -- review Scripts/sdk/prompts/content-verifier.md for calibration",
  diacritics:
    "Diacritics gate failing repeatedly -- check translation source data for encoding issues",
  date_validation:
    "Date validation gate failing repeatedly -- review event date format requirements",
};

const DEFAULT_SUGGESTION_TEMPLATE =
  "gate_name gate has failed COUNT consecutive times -- review pipeline configuration";
...
export function computePipelineMetrics(
  ...
  const finished = pipelineStates.filter(
    (s) => COMPLETED_STATUSES.has(s.status) || FAILED_STATUSES.has(s.status),
  );
  ...
  const firstAttemptPass = finished.filter(
    (s) => COMPLETED_STATUSES.has(s.status) && s.fix_attempts <= 1,
  ).length;
...
export function computeQueueDepth(
  ...
  for (const s of states) {
    ...
    const hasStuckLabel =
      labels.includes("fix-failed") || labels.includes("needs-human-review");
 ...
 */
export function detectPatternAlerts(states: WorkflowState[]): PatternAlert[] {
  ...
  for (const gate of allGates) {
    ...
    for (const s of sorted) {
      const hasGateFailure = s.findings.some((f) =>
        f.gates_failed.includes(gate),
      );
      if (hasGateFailure) {
        consecutiveCount++;
 ...
 */
export function buildHealthSectionHtml(metrics: HealthMetrics): string {
  if (metrics.message) {
    return `
    <!-- System Health -->
  ...
  if (metrics.pattern_alerts.length > 0) {
    const alertItems = metrics.pattern_alerts
      .map(
        (a) =>
          `<div style="padding:10px 14px;background:#fef2f2;border-left:3px solid #dc2626;border-radius:4px;margin:6px 0;">
...
function buildPipelineRow(label: string, m: PipelineMetrics): string {
