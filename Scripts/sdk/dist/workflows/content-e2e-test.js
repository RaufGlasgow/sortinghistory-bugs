...
import { loadWorkflowState } from "../lib/state.js";
import { runContentE2E, } from "./content-e2e.js";
...
function setupTestEnvironment(testName) {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "content-e2e-" + testName + "-"));
    ...
    PATHS.SESSION_REGISTRY = sessionRegistry;
    return {
        tempDir,
        fixturesPath,
        correctionsLogPath,
...
function teardownTestEnvironment(env) {
    // Restore PATHS
    PATHS.STATE_DIR = env.originalStateDir;
    ...
    try {
        ...
        console.warn("[test] WARNING: Could not clean up: " +
            (err instanceof Error ? err.message : String(err)));
    }
}
/** Collect state transitions by reading the state file after the E2E run */
async function getStateHistory(workflowId) {
...
async function testHappyPath() {
    ...
    try {
        ...
        const result = await runContentE2E({
        ...
        if (finalState) {
            ...
            if (finalState.status === "complete") {
                ...
                console.log("  FAIL: state file shows " +
                    finalState.status +
                    " (expected complete)");
                passed = false;
            }
        ...
        try {
            ...
            if (log.corrections.length > 0) {
                console.log("  PASS: " + log.corrections.length + " correction(s) in log");
            }
            else {
            ...
            console.log("  FAIL: could not read corrections log: " +
                (err instanceof Error ? err.message : String(err)));
            passed = false;
        }
        ...
        if (result.prNumber === null) {
            ...
            console.log("  FAIL: prNumber = " + result.prNumber + " (expected null for dry-run)");
            passed = false;
        }
    }
    finally {
...
async function testEscalation() {
    ...
    try {
            ...
            events: [
                {
                    ...
                    version: 1,
                    description: "This American historical event description is intentionally written to be extremely long with many extra unnecessary redundant superfluous words well beyond the maximum limit.",
                    year: 1776,
                    month: 7,
                    day: 4,
                    category: "US History",
        ...
        fs.writeFileSync(env.fixturesPath, JSON.stringify(escalationFixture, null, 2), "utf-8");
        ...
        const tempFixerPrompt = path.join(env.tempDir, "Scripts", "sdk", "prompts", "content-fixer.md");
        ...
        if (result.status === "escalated") {
            ...
            console.log("  FAIL: status = " + result.status + " (expected escalated)");
            passed = false;
        }
        ...
        if (result.fixAttempts === LIMITS.MAX_FIX_ATTEMPTS) {
            console.log("  PASS: fixAttempts = " + result.fixAttempts);
        }
        else {
            console.log("  FAIL: fixAttempts = " +
                result.fixAttempts +
                " (expected " +
        ...
        if (result.prNumber === null) {
            ...
            console.log("  FAIL: prNumber = " + result.prNumber + " (expected null)");
            passed = false;
        }
        ...
        if (finalState) {
            ...
            if (finalState.fix_attempts === LIMITS.MAX_FIX_ATTEMPTS) {
                console.log("  PASS: state file fix_attempts = " + finalState.fix_attempts);
            }
            else {
                console.log("  FAIL: state file fix_attempts = " +
...
// Main Test Runner
// ------------------------------------------------------------------
export async function runContentE2ETest() {
    console.log("=== Story 2.3: Content E2E Test Suite ===");
    console.log("Tests: 2 (Happy Path + Escalation)");
    console.log("Note: These tests spawn real AI subagents and will consume API credits.");
    console.log("");
    const results = [];
    // Test 1: Happy Path
