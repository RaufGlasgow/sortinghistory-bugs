...
import { LIMITS, PATHS, ROUTING, MODELS, FIXER_TOOLS } from "../config.js";
 ...
 */
function extractKeysFromSection(filePath, searchKeys, sectionStartLine, sectionEndLine) {
    const result = {};
...
function findLanguageSections(filePath) {
    ...
    let totalLines;
    try {
        const wcOutput = execSync("wc -l < " + JSON.stringify(filePath), { encoding: "utf-8", timeout: 5_000 }).trim();
 ...
 */
export function analyzeTranslationBug(issueBody, gameRepoPath) {
    ...
    try {
        const grepOutput = execSync("grep -o '\"daily_challenge_[a-z_]*\"' " + JSON.stringify(locHelperPath), { encoding: "utf-8", timeout: 10_000 }).trim();
 ...
 */
function translationToWorkflowFindings(findings) {
 ...
 */
function buildTranslationPrDescription(findings, workflowId, fixAttempts, issueNumber) {
    const keysList = findings.map(f => "- `" + f.key + "`: missing in " + f.missingIn.join(", ") +
        (f.wrongIn.length > 0 ? " | wrong in " + f.wrongIn.join(", ") : "")).join("\n");
...
function buildFixerUserPrompt(language, keys, locHelperPath) {
    ...
    if (missingKeys.length > 0) {
        instructions.push("MISSING KEYS (add to the " + language + " section):", missingKeys.map(k => "- `" + k.key + "`").join("\n"), "");
    }
 ...
 */
function recheckTranslationKeys(filePath, keys, languages, sections) {
 ...
 */
function createTranslationPR(gameRepoPath, branch, baseBranch, findings, workflowId, fixAttempts, issueNumber) {
    const repo = ROUTING.PRIVATE_REPO;
    ...
    try {
        ...
        execSync('git commit -m "fix(i18n): add missing translations for #' + issueNumber + '"', { cwd: gameRepoPath, encoding: "utf-8", timeout: 15_000 });
        ...
        const prOutput = execSync("gh pr create --repo " + repo +
            " --head " + branch +
            " --base " + baseBranch +
            ' --title "fix(i18n): missing translations for #' + issueNumber + '"' +
            " --body-file " + tmpFile, { cwd: gameRepoPath, encoding: "utf-8", timeout: 30_000 }).trim();
        try {
            fs.unlinkSync(tmpFile);
        }
        catch { /* cleanup */ }
 ...
 */
export async function runTranslationE2E(input) {
    ...
    // ---------------------------------------------------
    const state = await createWorkflowState("translation_verification", "dispatch", undefined, input.issueNumber);
    ...
    const findingsSummary = findings.map(f => "- `" + f.key + "` (English: \"" + f.englishValue + "\"): missing in " + f.missingIn.join(", ")).join("\n");
    postIssueComment(input.issueNumber, "## Translation Analysis Complete\n\n" +
    ...
    await saveSession(state.workflow_id, "sim-" + state.workflow_id, "fix_translation");
 ...
 */
export async function resumeTranslationE2E(workflowId, action, options) {
    ...
    const translationFindings = state.findings.map(f => {
    ...
    for (const f of translationFindings) {
        for (const lang of f.missingIn)
            allMissingLanguages.add(lang);
        for (const lang of f.wrongIn)
            allMissingLanguages.add(lang);
    }
    ...
    while (fixAttempt < LIMITS.MAX_FIX_ATTEMPTS && remainingFindings.length > 0) {
        ...
        for (const language of Array.from(allMissingLanguages)) {
            const keysForLang = remainingFindings
                ...
                fixType: (f.missingIn.includes(language) ? "missing" : "wrong"),
            }));
            if (keysForLang.length === 0)
                continue;
        ...
        remainingFindings = recheckTranslationKeys(locHelperPath, keysToTranslate, languagesToCheck, freshSections);
    ...
    if (remainingFindings.length === 0) {
        ...
        const pr = createTranslationPR(gameRepoPath, branch, baseBranch, translationFindings, workflowId, fixAttempt, state.issue_number ?? 0);
        if (pr) {
            await updateWorkflowState(workflowId, {
                status: "complete",
            ...
            if (state.issue_number) {
                postIssueComment(state.issue_number, "## Translation Fix Complete\n\n" +
    ...
    if (state.issue_number) {
        ...
        postIssueComment(state.issue_number, "## Translation Fix Escalated\n\n" +
            "After **" + fixAttempt + "** attempt(s), **" + remainingFindings.length +
            "** finding(s) still need manual translation.\n\n" +
            "### Remaining Issues\n" +
            remainingFindings.map(f => "- `" + f.key + "`: missing in " + f.missingIn.join(", ")).join("\n") + "\n\n" +
            "**Workflow:** `" + workflowId + "`\n\n" +
