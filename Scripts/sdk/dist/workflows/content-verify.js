...
function runInlineAutomatedChecks(events) {
    ...
    for (const event of events) {
        ...
        const hasCountryContext = COUNTRY_CONTEXT_TERMS.some(term => combinedText.includes(term));
 ...
 */
async function runAutomatedGates(filePath, repoRoot) {
    ...
    try {
        const output = execSync("python3 " + JSON.stringify(pythonScript) + " " + JSON.stringify(filePath) + " --staging --json", {
            cwd: gameRepo,
            encoding: "utf-8",
            timeout: 60000,
            env: { ...process.env },
 ...
 */
async function runAiVerificationBatch(events, systemPrompt, repoRoot, batchLabel) {
...
async function runAiVerification(events, repoRoot) {
    if (events.length === 0) {
    ...
    const merged = { events_checked: 0, events_passed: 0, events_failed: 0, results: [] };
...
// ------------------------------------------------------------------
export async function runContentVerify(input) {
    ...
    const { passedEvents, failures: autoFailures, usedPython } = await runAutomatedGates(resolvedPath, repoRoot);
