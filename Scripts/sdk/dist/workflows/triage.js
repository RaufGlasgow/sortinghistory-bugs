 ...
 */
export function extractTriageSignals(body, title) {
 ...
 */
function guessRelevantFiles(classification, context) {
    const files = [];
...
function fetchIssue(issueNumber) {
    ...
    try {
        ...
        issueJson = execSync("gh issue view " + issueNumber + " --repo " + repo + " --json title,body,labels,comments", { encoding: "utf-8", timeout: 30_000 }).trim();
    }
    catch (err) {
 ...
 */
export function extractOwnerComments(comments) {
    const ownerComments = [];
 ...
 */
export async function runRealTriage(input) {
    ...
    try {
        ...
        try {
            postIssueComment(issueNumber, "## Triage Failed\n\nCould not fetch this issue for triage.\n\n**Error:** " + errMsg);
        }
        catch {
    ...
    if (triageResult.classification === "content_error") {
        ...
        if (!category || (typeof category === "string" && category.trim() === "")) {
            ...
            postIssueComment(issueNumber, "## Triage: Content Error Detected\n\n" +
    ...
    try {
        ...
        postIssueComment(issueNumber, "## Routing Failed\n\n" +
...
export function buildClassificationComment(triage) {
...
function buildLogEntry(issueNumber, triage, action) {
    // Determine which gate fired
    let gate = "classification_route";
    let labels = [];
    if (action.type === "skip") {
        gate = "idempotency";
    }
...
function buildRoutingComment(triage, action) {
    switch (action.type) {
        case "dispatch":
