...
import { runContentFix } from "./content-fix.js";
...
function buildTestFindings(fixturesPath) {
    return [
        {
...
// Test Runner
// ------------------------------------------------------------------
export async function runContentFixTest() {
    ...
    try {
        ...
        console.log("");
        const fixOutput = await runContentFix({
            findings,
            correctionsLogPath,
            repoRoot: tempDir,
        });
        ...
        console.log("");
        const reVerifyResult = await runContentVerify({
        ...
        const unfixedErrors = reVerifyResult.summary.all_failures.filter(f => !fixedTitles.includes(f.title));
