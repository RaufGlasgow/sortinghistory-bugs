...
function fetchIssueContext(issueNumber) {
    const repo = ROUTING.PRIVATE_REPO;
    console.log("[bug-fix] Fetching issue #" + issueNumber + " from " + repo);
    // Fetch issue title and body
    let issueJson;
    try {
        issueJson = execSync("gh issue view " + issueNumber + " --repo " + repo + " --json title,body", { encoding: "utf-8", timeout: 30_000 }).trim();
    }
    catch (err) {
    ...
    let triageClassification = null;
    let triageSeverity = null;
    let triageConfidence = null;
    let triageReasoning = null;
    try {
        const commentsJson = execSync("gh issue view " + issueNumber + " --repo " + repo + " --json comments", { encoding: "utf-8", timeout: 30_000 }).trim();
        const commentsParsed = JSON.parse(commentsJson);
 ...
 */
export function extractTriageFromComments(comments) {
 ...
 */
function extractTriageFromCommentsLegacy(comments) {
    let triageComment = null;
    let triageClassification = null;
    let triageSeverity = null;
    let triageConfidence = null;
 ...
 */
export function extractOwnerReclassification(comments) {
    for (let i = comments.length - 1; i >= 0; i--) {
        const comment = comments[i];
 ...
 */
function inferFileExtensions(classification) {
 ...
 */
async function runQAOnly(input) {
    ...
    const state = await createWorkflowState("bug_fix", "dispatch", undefined, issueNumber);
 ...
 */
export async function runBugFix(input) {
    ...
    const summary = retryResult.fixSummary
        ? {
    ...
    if (retryResult.success) {
        return await handleSuccess(state.workflow_id, issueNumber, gameRepoPath, retryResult, summary, metrics);
    }
    else {
        return await handleFailure(state.workflow_id, issueNumber, gameRepoPath, retryResult, summary, metrics);
    }
}
// ------------------------------------------------------------------
// Success Handler (AC5)
 ...
 */
async function handleSuccess(workflowId, issueNumber, gameRepoPath, retryResult, summary, metrics) {
    console.log("");
 ...
 */
async function handleFailure(workflowId, issueNumber, gameRepoPath, retryResult, summary, metrics) {
    console.log("");
 ...
 */
function aggregateMetrics(retryResult) {
