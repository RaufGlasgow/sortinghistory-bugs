 ...
 */
import { ROUTING_FIXTURES } from "../tests/routing-fixtures.js";
import { decideRoute, executeRoute } from "../lib/routing.js";
import { logRoutingDecision } from "../lib/routing-log.js";
import { ROUTING, CLASSIFICATIONS, PATHS } from "../config.js";
import { generateTriageHandoff, buildFallbackHandoffComment } from "../lib/handoff-generator.js";
import { readRoutingLogForDate, buildLabelFallback, renderNeedsAttentionHtml, formatClassificationDisplay } from "../lib/digest-confidence.js";
 ...
 */
function validateAction(actual, expected) {
    const errors = [];
    // Type must match
    ...
    switch (expected.type) {
            ...
            if (expected.payload_values) {
                for (const [key, value] of Object.entries(expected.payload_values)) {
                    if (actual.payload[key] !== value) {
                        errors.push("payload." + key + ": got " + JSON.stringify(actual.payload[key]) +
        ...
        case "label": {
            ...
            if (JSON.stringify(actualLabels) !== JSON.stringify(expectedLabels)) {
                errors.push("labels: got [" + actualLabels.join(", ") + "], expected [" + expectedLabels.join(", ") + "]");
            }
            break;
        }
        case "label_and_state": {
            if (actual.type !== "label_and_state")
                break;
            if (actual.repo !== expected.repo) {
                errors.push("repo: got \"" + actual.repo + "\", expected \"" + expected.repo + "\"");
            }
            ...
            if (JSON.stringify(actualLabels2) !== JSON.stringify(expectedLabels2)) {
                errors.push("labels: got [" + actualLabels2.join(", ") + "], expected [" + expectedLabels2.join(", ") + "]");
            }
        ...
        case "handoff_to_dev": {
            ...
            if (JSON.stringify(actualLabelsH) !== JSON.stringify(expectedLabelsH)) {
                errors.push("labels: got [" + actualLabelsH.join(", ") + "], expected [" + expectedLabelsH.join(", ") + "]");
            }
...
export async function runRoutingTest() {
    ...
    let logSchemaCheckPassed = false;
    {
        ...
        if (origRepo !== undefined) {
            process.env.SDK_REPO_ROOT = origRepo;
        }
        else {
            delete process.env.SDK_REPO_ROOT;
        }
    ...
    let logNoSensitivePassed = false;
    {
        ...
        if (origRepo2 !== undefined) {
            process.env.SDK_REPO_ROOT = origRepo2;
        }
        else {
            delete process.env.SDK_REPO_ROOT;
        }
    ...
    const testGateValues = ["confidence", "unknown_classification", "idempotency", "classification_route"];
    ...
    try {
        ...
        const fixtureClassifications = new Set(allFixtures.map((f) => f.input.classification));
        for (const cls of CLASSIFICATIONS) {
    ...
    let costAwarenessPassed = false;
    {
        ...
        try {
            ...
            if (hasCostSection && hasCheapest && hasNeedsHumanDefault) {
                ...
                const missing = [];
                if (!hasCostSection)
                    missing.push("## Cost Awareness heading");
                if (!hasCheapest)
                    missing.push("cheapest/safest guidance");
                if (!hasNeedsHumanDefault)
                    missing.push("needs_human_review default when in doubt");
    ...
    let boundaryPassed = false;
    {
        ...
        try {
            ...
            if (crashMentionsGameplay && gameplayMentionsCrash) {
                ...
                const missing = [];
                if (!crashMentionsGameplay)
                    missing.push("crash_bug section doesn't mention gameplay_bug");
                if (!gameplayMentionsCrash)
                    missing.push("gameplay_bug section doesn't mention crash_bug");
    ...
    let triageWiringPassed = false;
    {
        ...
        try {
            ...
            if (hasIssueTitle && hasIssueBody && hasReasoning) {
                ...
                const missing = [];
                if (!hasIssueTitle)
                    missing.push("issue_title");
                if (!hasIssueBody)
                    missing.push("issue_body");
                if (!hasReasoning)
    ...
    let handoffWiringPassed = false;
    {
        ...
        try {
            ...
            if (hasGenerate && hasPostComment && hasRetry && hasFallback && hasDeliveryFailed && hasGenerationFailed) {
                ...
                const missing = [];
                if (!hasGenerate)
                    missing.push("generateTriageHandoff call");
                if (!hasPostComment)
                    missing.push("githubPostComment call");
                if (!hasRetry)
                    missing.push("retry logic");
                if (!hasFallback)
                    missing.push("buildFallbackHandoffComment call");
                if (!hasDeliveryFailed)
                    missing.push("delivery-failed label");
                if (!hasGenerationFailed)
                    missing.push("handoff-generation-failed label");
    ...
    let digestFallbackPassed = false;
    {
        ...
        if (labelResult.source !== "label_fallback")
            fallbackChecks.push("source should be label_fallback");
        if (labelResult.needs_attention.length !== 2)
            fallbackChecks.push("expected 2 flagged, got " + labelResult.needs_attention.length);
        if (labelResult.normal.length !== 1)
            fallbackChecks.push("expected 1 normal, got " + labelResult.normal.length);
        // Test renderNeedsAttentionHtml with flagged issues
        const html = renderNeedsAttentionHtml(labelResult.needs_attention, "https://github.com/RaufGlasgow/Sorting-History/issues/");
        if (!html.includes("Needs Attention"))
            fallbackChecks.push("HTML missing 'Needs Attention' heading");
        if (!html.includes("#201"))
            fallbackChecks.push("HTML missing issue #201");
        if (!html.includes("#203"))
            fallbackChecks.push("HTML missing issue #203");
        if (!html.includes("fef2f2"))
            fallbackChecks.push("HTML missing red background style");
        ...
        if (emptyHtml !== "")
            fallbackChecks.push("empty issues should return empty string, got: " + emptyHtml.slice(0, 50));
        // Test formatClassificationDisplay
        const display1 = formatClassificationDisplay("content_error", 0.92);
        if (display1 !== "content_error (92%)")
            fallbackChecks.push("display should be 'content_error (92%)', got: " + display1);
        const display2 = formatClassificationDisplay("ui_bug", -1);
        if (display2 !== "ui_bug (confidence unknown)")
            fallbackChecks.push("display should be 'ui_bug (confidence unknown)', got: " + display2);
