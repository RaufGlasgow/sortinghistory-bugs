...
function formatExtractedContext(ctx) {
...
export function generateHandoff(input) {
    ...
    sections.push("**Classification:** " + input.triageClassification + " | **Severity:** " + input.triageSeverity);
    ...
    sections.push("3. Review the failed approaches above to avoid repeating them");
...
function getClassificationGuidance(classification) {
    const guidance = {
        content_error: "This is a content error -- check the JSON data file for incorrect facts, dates, or descriptions",
        translation_error: "This is a translation error -- check the localized .json file for the affected language",
        ui_bug: "This is a UI bug -- check the SwiftUI View file for layout, styling, or rendering issues",
        gameplay_bug: "This is a gameplay bug -- check the game logic in the relevant ViewModel or Model file",
        crash_bug: "This is a crash -- check the stack trace in the original report and the relevant Swift file",
        performance_issue: "This is a performance issue -- profile the relevant code path and check for unnecessary redraws or allocations",
    };
 ...
 */
export function commitHandoff(result, gameRepoPath) {
...
function extractIssueNumber(filePath) {
 ...
 */
export function generateTriageHandoff(input) {
    const sections = [];
    // -- Header --
 ...
 */
export function buildFallbackHandoffComment(classification, confidence, severity, reasoning, error) {
    return [
 ...
 */
export function postHandoffComment(issueNumber, markdown) {
    ...
    try {
        ...
        const errMsg = err instanceof Error ? err.message : String(err);
        console.error("[handoff] WARNING: Failed to post handoff comment on issue #" + issueNumber + ": " + errMsg);
