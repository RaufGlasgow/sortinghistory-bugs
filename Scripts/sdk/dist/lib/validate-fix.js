 ...
 */
export function parseDiffFiles(diffContent) {
    const files = [];
 ...
 */
export function extractLangLabel(labels) {
 ...
 */
export function extractBugType(labels) {
    for (const label of labels) {
        if (label === "content-error")
            return "content-error";
        if (label === "translation-error")
 ...
 */
function checkLanguageMatch(bugType, langCode, files) {
    ...
    for (const file of files) {
        ...
        const matchesLang = file.includes("/" + langCode + "/") ||
        ...
        const isRootContentFile = bugType === "content-error" &&
            !ALL_LOCALE_CODES.some((code) => file.includes("/" + code + "/") ||
 ...
 */
function checkDiffVsClaim(issueBody, diffContent, files) {
...
function checkFileTypes(files) {
    ...
    if (forbidden.length > 0) {
        return {
            ...
            details: "Only .json content files are allowed. Forbidden files: " +
                forbidden.join(", "),
        };
    }
    return { valid: true };
 ...
 */
function checkEmptyDiff(files, diffContent) {
 ...
 */
export function validateFix(issueData, diffPath) {
    // Read the diff file
    let diffContent;
    try {
