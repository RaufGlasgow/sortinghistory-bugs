 ...
 */
function buildTriageHandoffHtml(handoff) {
    if (!handoff)
        return "";
    if (handoff.signals_found.length === 0 && handoff.signals_missing.length === 0)
        return "";
    const sections = [];
 ...
 */
function buildDiffHtml(diffText, prUrl) {
    ...
    for (const line of lines) {
        ...
        if (line.startsWith("Binary files") || line.startsWith("GIT binary patch")) {
            htmlParts.push(`<div style="padding:4px 10px;${hunkStyle}font-style:italic;">Binary file changed: ${escapeHtml(currentFile)}</div>`);
            renderedLines++;
            continue;
        }
        ...
        if (line.startsWith("---") || line.startsWith("+++")) {
            ...
            if (line.startsWith("+++")) {
                ...
                htmlParts.push(`<div style="padding:6px 10px;margin-top:12px;${headerStyle}font-size:13px;">${escapeHtml(filePath)}</div>`);
                renderedLines++;
            }
            continue;
        }
        // Hunk header: @@ -x,y +x,y @@
        if (line.startsWith("@@")) {
            htmlParts.push(`<div style="padding:2px 10px;${hunkStyle}font-size:12px;">${lineHtml}</div>`);
            renderedLines++;
            if (renderedLines >= DIFF_MAX_LINES) {
                truncated = true;
                break;
            }
            continue;
        ...
        htmlParts.push(`<div style="padding:1px 10px;${style}font-size:12px;line-height:1.5;white-space:pre-wrap;overflow-wrap:break-word;">${lineHtml}</div>`);
        renderedLines++;
        if (renderedLines >= DIFF_MAX_LINES) {
            truncated = true;
            break;
