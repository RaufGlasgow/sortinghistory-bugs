...
export function createToolRestrictionHook() {
    const restrictSwiftWrites = async (input, _toolUseID, _options) => {
...
export function createJsonValidationHook() {
    const validateJson = async (input, _toolUseID, _options) => {
...
onViolation) {
    ...
    const preWriteCounts = new Map();
    const recordPreWrite = async (input, _toolUseID, _options) => {
        if (input.hook_event_name !== "PreToolUse")
            return {};
        const preInput = input;
        const filePath = getFilePath(preInput.tool_input);
        if (!filePath?.includes("_pt.json") && !filePath?.includes("_pt-"))
            return {};
    ...
    const checkDiacritics = async (input, _toolUseID, _options) => {
        if (input.hook_event_name !== "PostToolUse")
            return {};
        const postInput = input;
        const filePath = getFilePath(postInput.tool_input);
 ...
 */
export function buildBugFixHooksConfig(gameRepoPath) {
    const blockBugFixWrites = async (input, _toolUseID, _options) => {
        if (input.hook_event_name !== "PreToolUse")
            return {};
        const preInput = input;
        const filePath = getFilePath(preInput.tool_input);
