 ...
 */
function loadSystemPrompt(profile) {
    ...
    if (!fs.existsSync(promptPath)) {
        throw new Error("QA system prompt not found: " + promptPath +
 ...
 */
function buildUserPrompt(input, singleProfile) {
    const parts = [];
 ...
 */
export function toVerdictEntry(verdict, attemptNumber) {
    const verdictMap = {
        approved: "pass",
 ...
 */
async function runSingleProfileReview(input, singleProfile) {
...
export async function runQAReview(input) {
    ...
    const finalVerdict = VERDICT_RANK[codeVerdict.verdict] >= VERDICT_RANK[contentVerdict.verdict]
        ? codeVerdict.verdict
        : contentVerdict.verdict;
    const finalRisk = RISK_RANK[codeVerdict.risk_level] >= RISK_RANK[contentVerdict.risk_level]
