...
import { computeHealthMetrics, computeQueueDepth, detectPatternAlerts, } from "../lib/health-metrics.js";
// ---------------------------------------------------------------------------
...
function makeFinding(gatesFailed) {
    return {
        event_id: "ev-001",
...
describe("Story 3.1: Health Metrics", () => {
    ...
    it("healthMetrics handles empty state directory gracefully", () => {
        ...
        assert.equal(metrics.message, "No pipeline data yet -- metrics will appear after the first completed cycle");
    });
});
