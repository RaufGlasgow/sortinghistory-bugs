...
describe("qa-retry-classification: non-retryable errors", () => {
    it('"Missing prompt file: qa-reviewer-code.md" → not retryable', () => {
        assert.equal(isRetryableQAError("Missing prompt file: qa-reviewer-code.md"), false);
    });
    it('"Invalid config: qaProfile must be code or content" → not retryable', () => {
        assert.equal(isRetryableQAError("Invalid config: qaProfile must be code or content"), false);
    });
    it('"Unexpected JSON parse error in response" → not retryable', () => {
        assert.equal(isRetryableQAError("Unexpected JSON parse error in response"), false);
    });
