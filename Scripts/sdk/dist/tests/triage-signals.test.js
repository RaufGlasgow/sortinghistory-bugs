...
describe("Story 3.5: extractTriageSignals", () => {
    it("finds category name, game mode, and CurrentScreen", () => {
        ...
        assert.ok(signals.found.some((s) => s.includes("category_name") && s.includes("ancient history")), "Should find 'ancient history' category signal");
        assert.ok(signals.found.some((s) => s.includes("game_mode") && s.includes("epic")), "Should find 'epic' game mode signal");
        assert.ok(signals.found.some((s) => s.includes("current_screen") && s.includes("BugReportView")), "Should find CurrentScreen signal");
    ...
    it("finds language/translation signals", () => {
        ...
        assert.ok(signals.found.some((s) => s.includes("language_signal") && s.includes("translation")), "Should find translation language signal");
        assert.ok(signals.found.some((s) => s.includes("game_mode") && s.includes("daily challenge")), "Should find daily challenge game mode signal");
    });
    it("reports missing signals for truly ambiguous reports", () => {
        ...
        assert.ok(signals.missing.some((s) => s.includes("no specific category")), "Should report missing category signal");
        assert.ok(signals.missing.some((s) => s.includes("no game mode")), "Should report missing game mode signal");
        assert.ok(signals.missing.some((s) => s.includes("no CurrentScreen")), "Should report missing CurrentScreen signal");
    });
});
// ---------------------------------------------------------------------------
...
describe("Story 3.5: triage fixtures validation", () => {
    it("new fixtures have correct structure", async () => {
        ...
        const newFixtures = TRIAGE_FIXTURES.filter((f) => ["test-K", "test-L", "test-M"].includes(f.id));
        ...
        for (const fixture of newFixtures) {
            ...
            assert.ok(fixture.expected_classification, `Fixture ${fixture.id} should have expected_classification`);
            assert.ok(fixture.expected_severity_range.length > 0, `Fixture ${fixture.id} should have expected_severity_range`);
        }
        // test-K: category + game mode context
        const testK = newFixtures.find((f) => f.id === "test-K");
        assert.ok(testK.report.includes("ancient history"), "test-K should mention ancient history category");
        assert.ok(testK.report.includes("CurrentScreen"), "test-K should include CurrentScreen field");
        // test-L: sharing context
        const testL = newFixtures.find((f) => f.id === "test-L");
        assert.ok(testL.report.includes("ShareCardView"), "test-L should mention ShareCardView");
        // test-M: translation + language context
        const testM = newFixtures.find((f) => f.id === "test-M");
        assert.ok(testM.report.includes("translation"), "test-M should mention translation");
        assert.ok(testM.report.includes("de"), "test-M should mention German language code");
    });
    it("all existing fixtures still have valid structure", async () => {
        ...
        for (const fixture of TRIAGE_FIXTURES) {
            ...
            assert.ok(fixture.expected_severity_range.length > 0, `Fixture ${fixture.id} should have at least one expected severity`);
        }
    });
});
