...
describe("BA-010.10 webhook: bug_type validation", () => {
    ...
    it("formatIssueBody includes Reporter Classification when bug_type present", () => {
        const body = formatIssueBody({ description: "Bug text", bug_type: "gameplay_bug" }, "BUG-TEST");
        assert.ok(body.includes("**Reporter Classification:** gameplay_bug"), "body should contain Reporter Classification line");
    });
    it("formatIssueBody omits Reporter Classification when bug_type absent", () => {
        const body = formatIssueBody({ description: "Bug text" }, "BUG-TEST");
        assert.ok(!body.includes("Reporter Classification"), "body should NOT contain Reporter Classification line");
    });
});
// ---------------------------------------------------------------------------
...
function adjustForHint(triageResult, reporterHint) {
...
describe("BA-010.10 triage: confidence adjustment on disagreement", () => {
    it("caps confidence at 0.70 when AI disagrees with reporter hint (confidence >= 0.70)", () => {
        const result = adjustForHint({ classification: "content_error", confidence: 0.85 }, "gameplay_bug");
    ...
    it("routes to needs_human_review when AI disagrees and confidence < 0.70", () => {
        const result = adjustForHint({ classification: "content_error", confidence: 0.55 }, "gameplay_bug");
    ...
    it("no adjustment when AI matches reporter hint", () => {
        const result = adjustForHint({ classification: "gameplay_bug", confidence: 0.92 }, "gameplay_bug");
        assert.equal(result.classification, "gameplay_bug");
        assert.equal(result.confidence, 0.92);
    });
    it("no adjustment when no reporter hint", () => {
        const result = adjustForHint({ classification: "content_error", confidence: 0.85 }, null);
        assert.equal(result.classification, "content_error");
