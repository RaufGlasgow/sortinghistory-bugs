...
describe("model-router: selectModels turn limits", () => {
    it("QA model is always <= fix model tier", () => {
        ...
        const profiles = ["content_simple", "content_complex", "code_simple", "code_complex"];
        const modelTier = {
            [HAIKU]: 1,
            [SONNET]: 2,
        };
        for (const profile of profiles) {
            for (let attempt = 1; attempt <= 3; attempt++) {
                ...
                assert.ok(modelTier[result.qaModel] <= modelTier[result.fixModel], `${profile} attempt ${attempt}: QA model (${result.qaModel}) should be <= fix model (${result.fixModel})`);
            }
        }
    });
