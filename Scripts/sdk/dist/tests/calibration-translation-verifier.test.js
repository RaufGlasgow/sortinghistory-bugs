...
import { runT0StructuralCheck, runT9DiacriticsCheck, isLikelyUntranslated, validatePortugueseDiacritics, } from "../workflows/translation-verify.js";
/**
...
const FIXTURE_PATH = path.resolve(SDK_ROOT, "tests", "fixtures", "calibration-translation", "calibration-test-set.json");
const RESULTS_PATH = path.resolve(SDK_ROOT, "tests", "fixtures", "calibration-translation", "calibration-results.json");
...
describe("Story 2.4a: Calibration Translation Verifier", () => {
    it("calibration test set has 10 German events: 5 good, 5 with planted errors", () => {
        ...
        assert.ok(fixture.portuguese_diacritics_events.length >= 3, "must have at least 3 Portuguese diacritics test events (got " +
    ...
    it("translation verifier catches untranslated string in German event", () => {
        ...
        const untranslatedEvent = results.ai_results.find(r => r.id === "cal_trans_error_1");
        assert.ok(untranslatedEvent, "cal_trans_error_1 (untranslated) must be in AI results");
        assert.equal(untranslatedEvent.overall_passed, false, "Untranslated event must FAIL. Details: " + untranslatedEvent.gates.T1_untranslated.details);
        assert.equal(untranslatedEvent.gates.T1_untranslated.passed, false, "Untranslated event must fail T1 gate. Details: " + untranslatedEvent.gates.T1_untranslated.details);
        assert.ok(untranslatedEvent.gates.T1_untranslated.code === "T1_UNTRANSLATED", "Must use code T1_UNTRANSLATED. Got: " + untranslatedEvent.gates.T1_untranslated.code);
    });
    // ---------------------------------------------------------------------------
    ...
    it("translation verifier catches wrong year in translation", () => {
        ...
        const wrongYearEvent = results.ai_results.find(r => r.id === "cal_trans_error_2");
        assert.ok(wrongYearEvent, "cal_trans_error_2 (wrong year) must be in AI results");
        assert.equal(wrongYearEvent.overall_passed, false, "Wrong year event must FAIL. Details: " + wrongYearEvent.gates.T3_factual.details);
        assert.equal(wrongYearEvent.gates.T3_factual.passed, false, "Wrong year event must fail T3 gate. Details: " + wrongYearEvent.gates.T3_factual.details);
        assert.ok(wrongYearEvent.gates.T3_factual.code === "T3_WRONG_YEAR", "Must use code T3_WRONG_YEAR. Got: " + wrongYearEvent.gates.T3_factual.code);
    });
    // ---------------------------------------------------------------------------
    ...
    it("translation verifier catches stripped diacritics in German event", () => {
        const fixture = loadFixture();
        const strippedEvent = fixture.translated_events.find(e => e.id === "cal_trans_error_3");
        ...
        assert.equal(t9Result.passed, false, "Stripped diacritics event must FAIL T9 gate. Details: " + t9Result.details);
        assert.ok(t9Result.code === "T9_STRIPPED", "Must use code T9_STRIPPED. Got: " + t9Result.code);
        // Also verify the calibration results recorded it
        const results = loadCalibrationResults();
        if (results) {
            const t9Failure = results.automated_gates.t9_failures.find(f => f.id === "cal_trans_error_3");
    ...
    it("translation verifier catches stale baseEnVersion", () => {
        const fixture = loadFixture();
        const staleEvent = fixture.translated_events.find(e => e.id === "cal_trans_error_4");
        assert.ok(staleEvent, "cal_trans_error_4 must exist in fixture");
        // Find matching English event
        const englishEvent = fixture.english_events.find(e => e.id === "de-hist-b2f3604d");
        ...
        assert.equal(t0Result.passed, false, "Stale baseEnVersion event must FAIL T0 gate. Details: " + t0Result.details);
        assert.ok(t0Result.code === "T0_STALE", "Must use code T0_STALE. Got: " + t0Result.code);
        // Verify baseEnVersion < English version
        assert.ok(staleEvent.baseEnVersion < englishEvent.version, "baseEnVersion (" + staleEvent.baseEnVersion + ") must be less than English version (" + englishEvent.version + ")");
    });
    // ---------------------------------------------------------------------------
    ...
    it("translation verifier catches tone/formality mismatch", () => {
        ...
        const toneEvent = results.ai_results.find(r => r.id === "cal_trans_error_5");
        assert.ok(toneEvent, "cal_trans_error_5 (tone mismatch) must be in AI results");
        assert.equal(toneEvent.overall_passed, false, "Tone mismatch event must FAIL. Details: " + toneEvent.gates.T5_tone.details);
        assert.equal(toneEvent.gates.T5_tone.passed, false, "Tone mismatch event must fail T5 gate. Details: " + toneEvent.gates.T5_tone.details);
        assert.ok(toneEvent.gates.T5_tone.code === "T5_TONE", "Must use code T5_TONE. Got: " + toneEvent.gates.T5_tone.code);
    });
    // ---------------------------------------------------------------------------
    ...
    it("translation verifier does not flag identical-across-languages words", () => {
        ...
        const lutherEvent = results.ai_results.find(r => r.id === "cal_trans_good_5");
        assert.ok(lutherEvent, "cal_trans_good_5 (Martin Luther) must be in AI results");
        assert.equal(lutherEvent.overall_passed, true, "Event with 'Martin Luther' in title must PASS (proper noun, not untranslated). " +
        ...
        for (const id of goodEventIds) {
            ...
            if (eventResult && !eventResult.overall_passed) {
                falsePositives++;
                falsePositiveDetails.push("'" + eventResult.title + "' (id=" + id + ") falsely flagged");
            }
        }
        assert.ok(falsePositives <= 1, "False positive rate must be < 20% (max 1 of 5 good events flagged). " +
        ...
        assert.equal(isUntranslated, false, "Good German event with 'Martin Luther' should NOT be flagged as untranslated");
    });
    // ---------------------------------------------------------------------------
    ...
    it("PostToolUse hook rejects write that strips Portuguese diacritics", () => {
        ...
        assert.ok(baselineResult.passed, "Correct Portuguese text must pass diacritics validation. " + baselineResult.details);
        assert.ok(baselineResult.density > 0.02, "Portuguese text should have > 2% diacritics density. Got: " +
        ...
        const strippedResult = validatePortugueseDiacritics(strippedText, baselineResult.density);
        assert.equal(strippedResult.passed, false, "Stripped Portuguese text must FAIL diacritics validation. " + strippedResult.details);
        // Verify the density actually decreased significantly
        assert.ok(strippedResult.density < baselineResult.density * 0.5, "Stripped density (" + (strippedResult.density * 100).toFixed(2) +
    ...
    it("PostToolUse hook allows write that preserves Portuguese diacritics", () => {
        ...
        assert.equal(result.passed, true, "Correct Portuguese text must PASS diacritics validation. " + result.details);
        ...
        assert.equal(modifiedResult.passed, true, "Slightly modified Portuguese text with preserved diacritics must PASS. " + modifiedResult.details);
        // Verify diacritics count is reasonable
        assert.ok(result.diacriticsCount > 10, "Portuguese test text should have significant diacritics (got " + result.diacriticsCount + ")");
    });
});
