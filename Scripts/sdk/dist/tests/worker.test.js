...
import { isSDKPipelineIssue, sanitizeText, validateBugReport, formatIssueBody, extractRejectionReason, } from "../lib/worker-utils.js";
// ---------- isSDKPipelineIssue ----------
describe("isSDKPipelineIssue", () => {
    ...
    it("true when mixed with other labels", () => {
        assert.equal(isSDKPipelineIssue([
            { name: "from-app" },
            { name: "content-error" },
            { name: "needs-triage" },
        ]), true);
    ...
    it("false for code-fix-eligible", () => {
        assert.equal(isSDKPipelineIssue([{ name: "code-fix-eligible" }, { name: "from-app" }]), false);
    });
    it("false for empty labels", () => {
        assert.equal(isSDKPipelineIssue([]), false);
    });
    it("false for unrelated labels", () => {
        assert.equal(isSDKPipelineIssue([{ name: "bug" }, { name: "high-severity" }]), false);
    });
});
// ---------- sanitizeText ----------
describe("sanitizeText", () => {
    ...
    it("handles script tags", () => {
        assert.equal(sanitizeText("<script>alert('xss')</script>Bug"), "alert('xss')Bug");
    });
...
describe("extractRejectionReason", () => {
    it("extracts reason: format", () => {
        assert.equal(extractRejectionReason("/reject reason: Wrong fix applied"), "Wrong fix applied");
    });
    it("extracts plain format", () => {
        assert.equal(extractRejectionReason("/reject The fix is wrong"), "The fix is wrong");
    });
