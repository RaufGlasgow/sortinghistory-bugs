...
import { createWorkflowState, updateWorkflowState, loadWorkflowState, listWorkflowStates, } from "../lib/state.js";
...
describe("createWorkflowState", () => {
    ...
    it("accepts optional category parameter", async () => {
        const state = await createWorkflowState("content_verification", "scheduled", "Sports History");
...
describe("updateWorkflowState", () => {
    ...
    it("throws when workflow does not exist", async () => {
        await assert.rejects(() => updateWorkflowState("nonexistent-id", { status: "complete" }), { message: "Workflow state not found: nonexistent-id" });
    });
...
describe("listWorkflowStates", () => {
    ...
    it("returns empty array when state directory does not exist", async () => {
        PATHS.STATE_DIR = path.join(tempDir, "nonexistent");
        const states = await listWorkflowStates();
        assert.deepEqual(states, []);
    });
