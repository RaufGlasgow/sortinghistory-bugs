...
async function main(): Promise<void> {
  ...
  const result: ContentVerificationResult = await runContentVerify({
  ...
  for (const event of fixtureData.events) {
    ...
    if (autoFailedTitles.has(title) || failedTitles.has(title)) continue;
    // Skip duplicates (second occurrence caught by D1)
    if (aiResults.some(r => r.title === title)) continue;
