 ...
 */
function buildContentTriageContext(
  category: string,
 ...
 */
function buildContentIssueBody(
  category: string,
  filePath: string,
  approvedFindings: WorkflowFinding[],
  correctionsLogPath: string,
 ...
 */
async function runContentRetryLoop(
  state: WorkflowState,
  category: string,
  gameRepoPath: string,
  options: {
  ...
  const issueBody = buildContentIssueBody(
    category,
    options.filePath,
    approvedFindings,
...
export async function finalizeWorkflow(
  ...
  if (retryResult.success) {
    ...
    console.log(
      "[content-e2e] Retry loop failed after " +
        retryResult.fixAttemptsUsed +
 ...
 */
function runStaleTranslationDetection(
  approvedFindings: WorkflowFinding[],
  category: string,
  gameRepoPath: string,
): StaleTranslationResult | null {
  ...
  let enEvents: Array<{ title: string; version?: number }>;
  try {
    const raw = fs.readFileSync(enFilePath, "utf-8");
    const data = JSON.parse(raw) as { events?: Array<{ title: string; version?: number }> };
  ...
  const aggregated: StaleTranslationResult = {
    hasStale: false,
    staleByLang: {},
    totalStale: 0,
    languagesChecked: [],
  ...
  for (const finding of approvedFindings) {
    ...
    const result = detectStaleTranslations(
      finding.event_title,
      newEnVersion,
      translationsDir,
      categoryFileName,
...
export async function runContentE2E(
  ...
  try {
    retryResult = await runContentRetryLoop(
      updatedState,
      category,
      repoRoot,
      {
