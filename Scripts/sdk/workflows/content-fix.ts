...
import { MODELS, FIXER_TOOLS, PATHS } from "../config.js";
...
function gateCodesToCorrectionType(codes: string[]): string {
  for (const code of codes) {
    if (code === "F1" || code === "F2") return "factual_error";
    if (code === "A1" || code === "A2") return "age_content";
    if (code === "D1" || code === "D2") return "duplicate_removal";
    if (code === "G0") return "parameter_fix";
    if (code === "P2" || code === "P3") return "parameter_fix";
    if (code === "P4") return "parameter_fix";
    if (code === "P5") return "parameter_fix";
  }
  return "clarity";
}
...
export async function runContentFix(input: ContentFixInput): Promise<ContentFixOutput> {
  ...
  const fixResults: ContentFixResult[] = [];
  let fixedCount = 0;
  let failedCount = 0;
