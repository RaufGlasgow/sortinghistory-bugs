...
function makeSampleTriage(): TriageResult {
  return {
...
describe("triage-contract: roundtrip", () => {
  it("build comment → extract triage data → all fields match original", () => {
    ...
    assert.equal(
      extracted!.extracted_context.expected_behavior,
      original.extracted_context.expected_behavior,
    );
    assert.equal(
      extracted!.extracted_context.actual_behavior,
      original.extracted_context.actual_behavior,
    );
  });
