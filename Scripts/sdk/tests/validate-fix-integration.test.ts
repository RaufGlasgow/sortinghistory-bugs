...
describe("Story 2.0c: validate-fix integration", () => {
  ...
  it("rejects diff containing forbidden file types (.swift)", () => {
    ...
    try {
      ...
      assert.strictEqual(result.reason, "forbidden-file-type",
        "Reason should be forbidden-file-type");
      assert.ok(result.details, "Details should be present");
      assert.ok(result.details!.includes("MainView.swift"),
  ...
  it("rejects translation-error diff that modifies wrong language path", () => {
    ...
    try {
      ...
      assert.strictEqual(result.reason, "language-mismatch",
        "Reason should be language-mismatch");
      assert.ok(result.details, "Details should be present");
      assert.ok(result.details!.includes("es"),
        "Details should mention expected language (es)");
    } finally {
      cleanupFile(diffPath);
    }
  });
  ...
  it("rejects translation-error issue missing lang label", () => {
    ...
    try {
      ...
      assert.strictEqual(result.valid, false,
        "Translation-error without lang label should fail");
  ...
  it("runValidateFixGate returns structured ValidationResult (AC9 direct call)", () => {
    ...
    try {
      ...
      const result = runValidateFixGate(
        42,
        "Content error in event 42",
        ["content-error"],
        tempDir,
      );
