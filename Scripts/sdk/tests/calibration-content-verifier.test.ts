...
const FIXTURE_PATH = path.resolve(
  SDK_ROOT, "tests", "fixtures", "calibration-content", "calibration-test-set.json",
);
const RESULTS_PATH = path.resolve(
  SDK_ROOT, "tests", "fixtures", "calibration-content", "calibration-results.json",
);

/** Load the calibration fixture file */
function loadFixture(): { category: string; events: Array<Record<string, unknown>> } {
  const raw = fs.readFileSync(FIXTURE_PATH, "utf-8");
  return JSON.parse(raw) as { category: string; events: Array<Record<string, unknown>> };
}
...
/** Shape of the AI calibration results file */
...
describe("Story 2.3a: Calibration Content Verifier", () => {
  ...
  it("validate_content.py catches P7 length violation in test set", () => {
    ...
    assert.ok(
      telephoneFailure.codes.includes("P2"),
      "First Telephone Call Made must be flagged with P2 (description too long). Got: [" + telephoneFailure.codes.join(", ") + "]",
    );
  });
  ...
  it("validate_content.py catches D1 duplicate in test set", () => {
    ...
    assert.ok(
      duplicateFailures.length >= 1,
      "at least one 'Declaration of Independence Signed' must be flagged as duplicate",
    );

    const d1Failure = duplicateFailures.find(f => f.codes.includes("D1"));
    assert.ok(
      d1Failure,
      "duplicate Declaration of Independence Signed must be flagged with D1. Got: [" +
  ...
  it("content verifier catches wrong-year factual error", () => {
    ...
    if (!results) {
      assert.fail(
        "Calibration results file not found at " + RESULTS_PATH +
    ...
    assert.equal(
      columbus.gate1_factual.passed, false,
      "Columbus Reaches Americas must FAIL Gate 1 (wrong year 1493 vs 1492). Details: " +
        columbus.gate1_factual.details,
    );
    assert.ok(
      columbus.gate1_factual.code === "F1" || columbus.gate1_factual.code === "F2",
      "Columbus failure must use code F1 or F2. Got: " + columbus.gate1_factual.code,
    );
  });
  ...
  it("content verifier catches wrong-country factual error", () => {
    ...
    assert.equal(
      teaParty.gate1_factual.passed, false,
      "Boston Tea Party must FAIL Gate 1 (wrong country context: Paris/France instead of Boston/America). Details: " +
        teaParty.gate1_factual.details,
    );
    assert.ok(
      teaParty.gate1_factual.code === "F1" || teaParty.gate1_factual.code === "F2",
      "Boston Tea Party failure must use code F1 or F2. Got: " + teaParty.gate1_factual.code,
    );
  });
  ...
  it("content verifier catches age-inappropriate content", () => {
    ...
    assert.equal(
      salem.gate2_age.passed, false,
      "Salem Witch Executions must FAIL Gate 2 (age-inappropriate graphic violence). Details: " +
        salem.gate2_age.details,
    );
    assert.ok(
      salem.gate2_age.code === "A1" || salem.gate2_age.code === "A2",
      "Salem failure must use code A1 or A2. Got: " + salem.gate2_age.code,
    );
  });
  ...
  it("content verifier does not flag known-good events as errors (max 1/5)", () => {
    ...
    assert.ok(
      falsePositives <= 1,
      "False positive rate must be < 20% (max 1 of good events flagged). " +
