...
import {
  submitVolumeTest,
  detectStuckIssues,
  extractDigestActionUrls,
  validateConcurrentStateFiles,
...
describe("Story 3.3: Launch Readiness", () => {
  it("volume test script submits 10 reports and collects responses", async () => {
    ...
    const result = await submitVolumeTest(
      "https://bug-webhook.emptycupmedia.workers.dev/api/bugs",
      TEST_BUG_REPORTS,
  ...
  it("digest URL extractor finds all action button links", () => {
    ...
    const nonPipelineUrls = buttons.filter(
      (b) => b.url === "https://sortinghistory.com" || b.url.includes("github.com"),
    );
  ...
  it("concurrent workflows create separate state files", () => {
    ...
    try {
      ...
      // Verify files exist
      assert.ok(
        fs.existsSync(path.join(stateDir, "cv-2026-03-09-001.json")),
        "Content verification state file should exist",
      );
      assert.ok(
        fs.existsSync(path.join(stateDir, "tv-2026-03-09-001.json")),
        "Translation verification state file should exist",
      );
      ...
      const cvState = JSON.parse(
        fs.readFileSync(path.join(stateDir, "cv-2026-03-09-001.json"), "utf-8"),
      );
      ...
      const tvState = JSON.parse(
        fs.readFileSync(path.join(stateDir, "tv-2026-03-09-001.json"), "utf-8"),
      );
  ...
  it("concurrent state file validator detects cross-contamination", () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(stateDir, "cv-2026-03-09-001.json"),
        JSON.stringify(contaminatedState, null, 2),
        "utf-8",
      );
      ...
      assert.ok(
        result.contamination_details[0].includes("issue_number is 161, expected 160"),
        "Should explain the contamination",
      );
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
